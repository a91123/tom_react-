...
function Rows(props) {
  ...
  const favHandle = (sid) => {
    ...
    const FavLength = getFav.filter((value, i) => v.sid == value.sid)
          ...
          >
            ...
            {fav ? (
              <svg
                ...
                onClick={() => {
                  if (localStorage.heart) {
                    const getHeart = JSON.parse(localStorage.getItem('heart'))
              ...
              <svg
                ...
                viewBox="0 0 24 24"
                onClick={() => {
                  if (localStorage.heart == null) {
                    localStorage.setItem('heart', JSON.stringify([data]))
                  } else {
                    const getHeart = JSON.parse(localStorage.getItem('heart'))
                    const addHeart = [data, ...getHeart]
                    localStorage.setItem('heart', JSON.stringify(addHeart))
                  }
